...
- 支持 Webhook Body 模板，使用 `__CONTENT__` 作为日志正文占位符，使用可选的 `__TIME__` 作为日志时间占位符。
...
发布包已包含默认 `config.json`，默认 `WindowTitle` 为 `MaaEnd `（包含末尾空格，不能省略），程序会查找标题中包含该文本的窗口，并从 `运行日志` 下方开始识别。后续启动时，程序会自动检查配置；如果配置完整且合法，会直接开始运行，不再要求确认。
...
- `WebhookBody`：推送请求体模板，必须包含 `__CONTENT__`，可选包含 `__TIME__`。
...
- 目标窗口：标题中包含 `MaaEnd ` 的窗口，例如 `MaaEnd v2.13.0`。注意反引号内包含末尾空格，默认配置不能省略。
- 锚点文本：`运行日志`。
...
- 匹配方式：程序会优先精确匹配窗口标题；默认短标题为 `MaaEnd `（包含末尾空格），可匹配到 `MaaEnd v2.13.0` 这类完整标题。
...
| `WebhookBody` | Webhook 请求体模板 | 非空，必须包含 `__CONTENT__`；可选包含 `__TIME__` |
...
Webhook 推送时，`__CONTENT__` 只会替换为日志正文，不再包含时间；如果模板中存在 `__TIME__`，程序会用识别到的时间字符串替换它。
...
- `WindowTitle` 可以填写完整标题，也可以填写稳定短标题；默认短标题必须为 `MaaEnd `，末尾空格不能省略。
